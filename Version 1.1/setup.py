    #Digital Dead Drop Setup - Pre transmission email encryption setup and configuration
...
DigiDrop = open('DigiDrop.py','rb')
runFile = DigiDrop.read()
DigiDrop.close()

def EncDriveSecret(pDrive, vDrive, Name):
    preSeeded = pDrive + vDrive
    salt = Name
    sRun = len(salt)
    sText = ''
...
sOne = DriveInfo[int(pDrive)][2]
if vYes == 'y':
    sTwo = DriveInfo[int(vDrive)][2]
else:
    sTwo = DriveInfo[int(pDrive)][2]
Name = DriveInfo[int(pDrive)][3]
dHash = EncDriveSecret(sOne, sTwo, Name)
...
encFile.close()
iFile = open('DigiDrop.py','wb')
iFile.write(runFile)
iFile.close()
dupeKey = raw_input('Do you need to setup another USB Key(y/n)? ')
dupeKey = dupeKey.lower()
if dupeKey == 'y':
    while dupeKey == 'y':
        raw_input("Please verify that your physical drive has been inserted and named before pressing enter to continue.")
        runs = 0
        vYes = raw_input("Will you be using a mounted encrypted disc(y/n)? ")
        vYes = vYes.lower()
        if vYes == 'y':
